...
class QuantumLink:
    ...
    # QuRA physics patch: disable Qiskit lookup table; use Werner formula.
    USE_QISKIT = False
    FIDELITIES_QISKIT = None

    # QuRA physics: entanglements expire after this many timeslots.
    ...
    def __init__(self, start, end, fidelity, cost=0, creation=None):
        ...
        self._decay_calls = 0  # QuRA physics: expire after ENTANGLEMENT_LIFETIME steps

    def decay(self, decay_factor):
        self._decay_calls += 1
        if self._decay_calls >= QuantumLink.ENTANGLEMENT_LIFETIME:
            self.fidelity = 0
            return
    ...
    def get_swap_fidelity(fidelity1, fidelity2, gate_error=0, random_index=None):
        # QuRA physics: Werner-state swap model (AlgorithmBase.fidelityAfterSwap)
        fidelity = fidelity1 * fidelity2 + (1 - fidelity1) * (1 - fidelity2) / 3.0

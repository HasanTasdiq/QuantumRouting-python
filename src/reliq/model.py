...
class ConcatAggregation(nn.Module):
    def __init__(self, agg: str, mask_eye: bool, max_degree: int) -> None:
        super().__init__()
        self.agg = agg
        self.mask_eye = mask_eye
    ...
    def forward(self, node_features, node_adjacency):
        ...
        message_size = node_features.shape[2]

        entries_other_messages = math.floor(message_size / (self.max_degree + 1))
        entries_own_message = message_size - self.max_degree * entries_other_messages

        mask_sparse, mask_weights = dense_to_sparse(node_adjacency)
        node_features_sparse = torch.reshape(node_features, (batch_size * node_count, message_size))
        empty_message = torch.ones((1, message_size), dtype=node_features_sparse.dtype, device=node_features_sparse.device) * -1
        node_features_sparse = torch.concat((node_features_sparse, empty_message))

        own_message_filter = mask_sparse[0, :] == mask_sparse[1, :]
        own_messages = mask_sparse[:, own_message_filter]
        other_messages = mask_sparse[:, torch.logical_not(own_message_filter)]
        message_count = torch.bincount(mask_sparse[0])
        corrected_own_message_locations = torch.zeros((message_count.shape[0]), dtype=mask_sparse.dtype,
                                                      device=mask_sparse.device)
        maximum_message_count = torch.ones(message_count.shape[0], dtype=torch.int16, device=node_features_sparse.device) * (self.max_degree + 1)
        corrected_own_message_locations[1: message_count.shape[0]] = torch.cumsum(
            maximum_message_count[0: maximum_message_count.shape[0] - 1], dim=0)

        corrected_own_message_filter = torch.zeros(batch_size * (self.max_degree + 1) * node_count, dtype=own_message_filter.dtype,
                                       device=own_message_filter.device)
        corrected_own_message_filter[corrected_own_message_locations] = True
        missing_messages_count = maximum_message_count - message_count
        corrected_other_message_filter = torch.zeros(batch_size * (self.max_degree + 1) * node_count, dtype=own_message_filter.dtype,
                                                     device=own_message_filter.device)
        corrected_empty_message_filter = torch.zeros(batch_size * (self.max_degree + 1) * node_count, dtype=own_message_filter.dtype,
                                                     device=own_message_filter.device)

        for i in range(1, missing_messages_count.max() + 1):
            empty_message_mask = missing_messages_count >= i
            corrected_empty_message_filter[corrected_own_message_locations + (self.max_degree + 1) - i] = empty_message_mask

        corrected_mask_sparse = torch.zeros((batch_size * (self.max_degree + 1) * node_count), dtype=mask_sparse.dtype, device=mask_sparse.device)
        # corrected_mask_sparse[0] = torch.floor(torch.arange(0, (self.max_degree + 1) * node_count, 1) / (self.max_degree + 1))
        corrected_mask_sparse[corrected_own_message_filter] = own_messages[1]
        corrected_mask_sparse[corrected_empty_message_filter] = node_features_sparse.shape[0] - 1
        corrected_mask_sparse[torch.logical_and(torch.logical_not(corrected_own_message_filter), torch.logical_not(corrected_empty_message_filter))] = other_messages[1]

        messages = node_features_sparse[corrected_mask_sparse].reshape((batch_size, node_count, self.max_degree + 1, message_size))
        reduced_own_messages = messages[:,:,0,0:entries_own_message]
        reduced_other_messages = messages[:, :, 1 : self.max_degree + 1, 0:entries_other_messages].reshape((batch_size, node_count, self.max_degree * entries_other_messages))
        concatenated_messages = torch.cat((reduced_own_messages, reduced_other_messages), dim=2)
...
class NetMon(nn.Module):
    def __init__(
        ...
        if agg_type == "sum" or agg_type == "mean":
            ...
            self.aggregate = ConcatAggregation(agg="filter", mask_eye=False, max_degree=self.max_degree)
            self.aggregation_def_type = 0

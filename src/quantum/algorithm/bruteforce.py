...
class BruteForce():
    ...
    def get_paths(self):
        ...
        # print(G.nodes())
        print('get path edge len ' , len(G.edges()))
        req = reqs[0]
        reqs.pop(0)
        all_paths = []
        paths = get_k_paths(G , req[0] , req[1])
        # print('in main')
        # print([p for p in paths])
        # print('len ent ' , len([l for l in self.topo.links if l.entangled]))
        # print('edge ' , len(G.edges()))
        # print(G.edges())
        # print([[(l.n1.id, l.n2.id) for l in self.topo.links if l.entangled]])
        print(len(paths))
        for path in paths:
            # print('in main,  path:' , path)
            G2 = copy.deepcopy(G)
            ...
            for i in range(len(path) - 1):
                edge = (min((path[i] , path[i+1])) , max((path[i] , path[i+1])))
                # print('before remove edge ' , len(G2.edges()))
                G2.remove_edge(edge[0]  , edge[1])
                # print('after remove edge ' , len(G2.edges()))

            comb = [G2 , path]
            all_paths.append(comb)
        if not len(paths):
            all_paths.append([copy.deepcopy(G)])
        ...
        # print(ids[t])
        # print([path for path in opt_comb])
        s_path = shortest_approach(copy.deepcopy(G) , copy.deepcopy(self.reqs))
        print('-------shortest -------')
        print(len(s_path))
        ret = {(self.topo.nodes[r[0]] , self.topo.nodes[r[1]]) : [] for r in self.reqs}
        for path in opt_comb:
            ret[(self.topo.nodes[path[0]] , self.topo.nodes[path[-1]])] = path

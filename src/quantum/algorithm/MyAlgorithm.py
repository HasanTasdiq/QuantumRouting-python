...
class MyAlgorithm(AlgorithmBase):
    ...
    def resetFailedRequestFor01(self, usedLinks):                   # 第一段傳失敗
        ...
        for req in self.requestState:
            ...
            if success == 0:
                if requestInfo.state == 0 or requestInfo.state == 1:    # 0, 1
                    self.resetFailedRequestFor01(usedLinks)
                elif requestInfo.state == 2:                            # 2
                    requestInfo.savetime += 1
                    self.resetFailedRequestFor01(usedLinks)
                    if requestInfo.savetime > self.r:   # 超出k儲存時間 重頭送 重設req狀態
                        self.resetFailedRequestFor2(self, requestInfo, usedLinks)  
                continue
...
if __name__ == '__main__':
    ...
    # b = []
    # b.append(RequestInfo(0, None, 3, [], False, 0)) 
    # a = b[0]
    # a.state = 1

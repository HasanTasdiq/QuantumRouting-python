...
class MERR(AlgorithmBase):
    ...
    def LP1(self):
        ...
        # t = m.addVars(numOfSDpairs, lb = 0, vtype = gp.GRB.INTEGER, name = "t")
    ...
    def weightofLink(self , u , v):
        ...
        return 1/capacity
    def calculate_lmax(self , initial_fidelity, required_fidelity):
        if self.lmax:
            return self.lmax
        """
        Calculate the maximum number of links (lmax) required to achieve the required fidelity.

        Parameters:
            initial_fidelity (float): The fidelity of a single link.
            required_fidelity (float): The desired overall fidelity of the chain.

        Returns:
            int: The maximum number of links (lmax) to achieve the required fidelity.
        """
        if initial_fidelity <= 0 or initial_fidelity >= 1:
            raise ValueError("Initial fidelity must be between 0 and 1 (exclusive).")
        if required_fidelity <= 0 or required_fidelity >= 1:
            raise ValueError("Required fidelity must be between 0 and 1 (exclusive).")

        current_fidelity = initial_fidelity
        lmax = 1

        while current_fidelity > required_fidelity:
            lmax += 1
            current_fidelity = current_fidelity * initial_fidelity + (1 - current_fidelity) * (1 - initial_fidelity)/3
        self.lmax = lmax - 1
        return self.lmax
    def LP2(self):
        ...
        l_max = self.calculate_lmax(self.topo.initial_fidelity, self.topo.fidelity_threshold)
        # l_max = 100
        ...
        for i in range(numOfSDpairs):
            ...
            for k in range(numOfFlow[i]):
                ...
                m.addConstr(gp.quicksum((f[i][k][u][v] + f[i][k][v][u])   for (u,v) in edgeIndices)  <= l_max)
                # m.addConstr(gp.quicksum((f[i][k][u][v] )   for (u,v) in edgeIndices)  <= l_max)
    ...
    def ELS(self):
        ...
        for sd in toremovesd:
            self.srcDstPairs.remove(sd)
        print('===== len sdpair after ' , len(self.srcDstPairs))
    ...
    def findPathsForEPS(self, SDpair, k):
        ...
        while self.DijkstraForEPS(SDpair, k):
            ...
            print('==!!=!=!+===!!!!!!!!!!==========path for EPS ' , [p.id for p in path] , len(path)  , self.topo.fidelity_threshold )
